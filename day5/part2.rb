...
while opcode_copy[memory_index] != '99'
  ...
  opcode, modes = split_opcode_instruction(opcode)
  number_of_params = 0
  case opcode
  when '1'
    number_of_params = 4
    opcode_copy[opcode_copy[memory_index + 3].to_i] =
      (fetch_values(modes[-1], opcode_copy[memory_index + 1], opcode_copy).to_i + fetch_values(modes[-2], opcode_copy[memory_index + 2], opcode_copy)).to_s
  when '2'
    number_of_params = 4
    opcode_copy[opcode_copy[memory_index + 3].to_i] =
      (fetch_values(modes[-1], opcode_copy[memory_index + 1], opcode_copy).to_i * fetch_values(modes[-2], opcode_copy[memory_index + 2], opcode_copy)).to_s
  when '3'
    number_of_params = 2
    opcode_copy[opcode_copy[memory_index + 1].to_i] = INPUT
  when '4'
    number_of_params = 2
    output << opcode_copy[opcode_copy[memory_index + 1].to_i]
  when '5'
    if fetch_values(modes[-1], opcode_copy[memory_index + 1], opcode_copy) != 0
      memory_index = fetch_values(modes[-2], opcode_copy[memory_index + 2], opcode_copy)
    else
      number_of_params = 3
    end
  when '6'
