...
def move(array, direction, steps)
  axe, operator = movement(direction)
  steps.times.each do
    new_position = array.last.dup
    new_position[axe] = new_position[axe].send(operator, 1)
    array << new_position
  end
end

def array_position(wire)
  array = [[0, 0]]
  wire.each do |direction|
    move(array, direction[0], direction[1..-1].to_i)
  end
  array
end

position_w1 = array_position(w1)
position_w2 = array_position(w2)

crossing = (position_w1 & position_w2)
crossing.shift
first_crossing = crossing.first
last_crossing = crossing.last

first_crossing = position_w2.index(first_crossing) + position_w1.index(first_crossing)
last_crossing = position_w2.index(last_crossing) + position_w1.index(last_crossing)

p [first_crossing, last_crossing].min
